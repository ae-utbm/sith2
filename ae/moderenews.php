    ...
    && ($_REQUEST['action'] == "mod"))
{
  ...
  if (isset($_REQUEST['accept']))
    {
      $news->load_by_id($id);
      //$ret = $news->validate ($site->user->id);
      $news->save_news(
			     $_REQUEST['asso'],
			     $_REQUEST['nws_title'],
			     $_REQUEST['nws_sum'],
			     $_REQUEST['nws_cts'],
			     true,$site->user->id,$_REQUEST['type']);
    ...
    ($_REQUEST['action'] == "edit"))
{
  ...
  $assoces_sql = new requete ($site->db,
			      "SELECT `id_asso`, `nom_asso` FROM `asso`");
  $assoces[0] = "-";
  for ($i = 0; $i < $assoces_sql->lines; $i ++)
    {
      $res = $assoces_sql->get_row();
      $assoces[$res['id_asso']] = $res['nom_asso'];
    }
  /* titre */
  $form->add_text_field ("nws_title",
			 "Titre de la nouvelle :",
			 $news->titre,
			 true,
			 "80");
  /* resume */
  $form->add_text_area ("nws_sum",
			"Resume :",
			$news->resume,
			80,
			2);
