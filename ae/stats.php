...
$req = new requete($site->db,"SELECT ROUND(SUM(ae_utl)*100/$cotisants,1) AS `count`, ROUND(SUM(ae_utl)*100/SUM(IF (ancien_etudiant_utl='0' OR ae_utl='1',1,0)),1) AS `taux`,IF(`utl_etu_utbm`.`promo_utbm` IS NULL,0,`utl_etu_utbm`.`promo_utbm`)AS `promo` FROM `utilisateurs` LEFT JOIN `utl_etu_utbm` USING(`id_utilisateur`) GROUP BY `promo` ORDER BY `count` DESC");
...
$req = new requete($site->db,"SELECT ROUND(SUM(ae_utl)*100/$cotisants,1) AS `count`, ROUND(SUM(ae_utl)*100/SUM(IF (ancien_etudiant_utl='0' OR ae_utl='1',1,0)),1) AS `taux`,IF(`utl_etu_utbm`.`branche_utbm` IS NULL,'Autre',`utl_etu_utbm`.`branche_utbm`) AS `branche` FROM `utilisateurs` LEFT JOIN `utl_etu_utbm` USING(`id_utilisateur`) GROUP BY `branche` ORDER BY `count` DESC");
...
$req = new requete($site->db,"SELECT COUNT(DISTINCT ae_cotisations.id_utilisateur) FROM `ae_cotisations` INNER JOIN site_sessions USING(`id_utilisateur`) WHERE `date_fin_cotis` > NOW() AND `derniere_visite`>'$debut_semestre'");
list($nbutil) = $req->get_row();
$cts->add_paragraph("".round($nbutil*100/$cotisants,1)."% ont consulté* le site ce semestre");

$req = new requete($site->db,"SELECT COUNT(DISTINCT ae_cotisations.id_utilisateur) FROM `ae_cotisations` INNER JOIN site_sessions USING(`id_utilisateur`) WHERE `date_fin_cotis` > NOW() AND DATEDIFF(NOW(),`derniere_visite`) < 30 ");
list($nbutil) = $req->get_row();
$cts->add_paragraph("".round($nbutil*100/$cotisants,1)."% ont consulté* le site dans les 30 derniers jours");

$cts->add_paragraph("(* cela ne concerne que les ouvertures de session pour le moment)");
...
$req = new requete($site->db,"SELECT COUNT(DISTINCT utilisateurs.id_utilisateur) FROM `utilisateurs` INNER JOIN site_sessions USING(`id_utilisateur`) WHERE `ancien_etudiant_utl`='0' AND `derniere_visite`>'$debut_semestre'");
list($nbutil) = $req->get_row();
$cts->add_paragraph("".round($nbutil*100/$total,1)."% ont consulté* le site ce semestre");

$req = new requete($site->db,"SELECT COUNT(DISTINCT utilisateurs.id_utilisateur) FROM `utilisateurs` INNER JOIN site_sessions USING(`id_utilisateur`) WHERE `ancien_etudiant_utl`='0' AND DATEDIFF(NOW(),`derniere_visite`) < 30 ");
list($nbutil) = $req->get_row();
