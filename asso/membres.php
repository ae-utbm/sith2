...
if ( $site->user->id < 1 || ( !$site->user->ae && !$can_admin ) )
	$limited = true;
...
if ( $_REQUEST["view"] == "trombino" || $limited )
{
  ...
  while ( $row = $req->get_row() )
  {
    ...
    if ( file_exists($topdir."images/matmatronch/".$row['id_utilisateur'].".identity.jpg") )
      $img = $topdir."images/matmatronch/".$row['id_utilisateur'].".identity.jpg";
    elseif ( file_exists($topdir."images/matmatronch/".$row['id_utilisateur'].".jpg") )
  ...
  if ( $limited && $site->user->id < 1 && !is_null($asso->id_parent) )
  {
    $cts->add_title(2,"Plus d'informations");
    $cts->add("Pour acceder à plus d'informations, veuillez vous connecter."); 
  }
}
