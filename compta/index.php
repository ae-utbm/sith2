...
$cts = new contents ("Accueil Compta v2");
$cts->add_paragraph ("Bienvenue sur le site de la compta.");
$cts->add_paragraph (wikilink("compta","Aide en ligne"));
$site->add_contents ($cts);

if ( !$site->user->is_in_group("compta_admin") )
{
	foreach($assos as $key=>$val)$assos_keys[]=$key;
	$filter = " AND `asso`.`id_asso` IN (".implode(",",$assos_keys).") ";
}	
$cts = new contents ("Comptes et classeurs");
...
$req_sql = new requete ($site->db,
			...
			"WHERE `cpta_classeur`.`ferme`=0 $filter " .
