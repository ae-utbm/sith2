...
{
	//$subfrm->add_radiobox_field($stock_n,false,array("nlim"=>"Illimité","lim"=>"Limité à"),($stock==-1)?"nlim":"lim");
	...
	//add ( $frm, $check=false, $option=false, $checked=false, $value=false, $line=false, $onoff=false,$on=true )
...
else if ( $_REQUEST["action"] == "addtype" && ($assocpt->id > 0) )
{
	...
	$typeprod->ajout( $_REQUEST["nom"], $_REQUEST["id_action"], $assocpt->id, $_REQUEST["url_logo"],$_REQUEST["description"] );
}
/*
...
else if ( $_REQUEST["action"] == "upproduit" && ($produit->id > 0) && ($typeprod->id > 0) )
{
    ...
    $produit->modifier ($typeprod->id,
		  ...
		  $_REQUEST['id_action'],
		  $_REQUEST['code_barre'],
		  $stock_global,$_REQUEST["url_logo"],$_REQUEST["description"],$assocpt->id,
		  $_REQUEST["a_retirer"],
		  $_REQUEST["postable"],
		  $_REQUEST["frais_port"]);
}
...
{
	$typeprod->modifier( $_REQUEST["nom"], $_REQUEST["action"], $assocpt->id, $_REQUEST["url_logo"], $_REQUEST["description"] );
}
...
elseif ( $_REQUEST["page"] == "addtype" )
{
	...
	$frm->add_text_field("url_logo","Logo");
...
elseif ( $_REQUEST["page"] == "addproduit" )
{
	...
	$frm->add_text_field("url_logo","Logo");
	$frm->add_info("Vous pouvez utiliser le système de fichier du site de l'ae pour le logo. Placez l'image dans la <a href=\"".$topdir."d.php?id_folder=198\">catégorie fichier</a>, puis utilisez la syntaxe suivante: dfile://id");
	$frm->add_text_area("description","Description");
...
elseif ( $produit->id > 0 )
{
	...
	$frm->add_text_field("url_logo","Logo",$produit->url_logo);
	$frm->add_info("Vous pouvez utiliser le système de fichier du site de l'ae pour le logo. Placez l'image dans la <a href=\"".$topdir."d.php?id_folder=198\">catégorie fichier</a>, puis utilisez la syntaxe suivante: dfile://id");
	$frm->add_text_area("description","Description",$produit->description);
...
elseif ( $typeprod->id > 0 )
{
	...
	$frm->add_text_field("url_logo","Logo",$typeprod->url_logo);
