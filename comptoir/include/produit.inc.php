...
/** @todo � adapter un nouveau syst�me *********************************/


/** @file
 *
 * @brief d�claration de la classe produit
 */
...
class produit
{
  ...
  // ACL, limitation de la vente à un groupe
  /** Vente authorisé à ce seul groupe (si non null) */
  var $id_groupe;
  ...
  */
   ...
   *
   */
  function produit ($db,$dbrw=false)
  {
    $this->db = $db;
    $this->dbrw = $dbrw;
    $this->id = -1;
		     ...
		     $prix_vente_cotisant=0)
  {
    ...
    $this->prix_vente_cotisant = intval($prix_vente_cotisant);
    ...
    $req = new insert ($this->dbrw,
		       ...
		       array("id_typeprod" => $this->id_type,
			     ...
			     "prix_vente_prod_cotisant" => $this->prix_vente_cotisant,
  ...
  function modifier_asso ( $asso)
  {

    if ( $this->determine_deja_vendu() )
      return false;

    $this->id_assocpt = $association->id;

    $req = new update ($this->dbrw,
		       "cpt_produits",
		       array("id_assocpt" => $this->id_assocpt),
		       array("id_produit" => $this->id));
    if ( !$req )
    	return false;
  ...
  function charge_row ($row)
  {
    ...
    $this->prix_vente_cotisant = $row['prix_vente_prod_cotisant'];
    ...
    $this->url_logo = $row['url_logo_prod'];
