...
class venteproduit
{
	...
	/** @brief constructeur
	 *
	 * @param db un objet mysqlae
	 *
	 */
	function venteproduit($db,$dbrw=false)
	{
		$this->db = $db;
		$this->dbrw = $dbrw;
	}
	...
	function load_by_id ( $id_produit, $id_comptoir )
	{
	 ...
	 * @private
	 */
	function _delta_verrou ( $client, $delta, $cot )
	{
		$req = new requete($this->dbrw,
				   "SELECT `quantite` FROM `cpt_verrou` ".
				   "WHERE `id_produit` = '".intval($this->produit->id)."' ".
				   "AND `id_comptoir` = '".intval($this->comptoir->id)."' ".
				   "AND `id_utilisateur` = '".intval($client->id)."' ".
				   "AND `prix_cotisant` = '0'");
		$req2 = new requete($this->dbrw,
		                   "SELECT `quantite` FROM `cpt_verrou` ".
				   "WHERE `id_produit` = '".intval($this->produit->id)."' ".
				   "AND `id_comptoir` = '".intval($this->comptoir->id)."' ".
				   "AND `id_utilisateur` = '".intval($client->id)."' ".
		...
		if ( $req->lines )
		{
			...
			if ( $qte+$qte2+$delta <= 0 )
			{
				$req = new requete($this->dbrw,
					   ...
					   "AND `id_utilisateur` = '".intval($client->id)."'");
				$req2 = new requete($this->dbrw,
				           "DELETE FROM `cpt_verrou_cotisant` ".
					   "WHERE `id_produit` = '".intval($this->produit->id)."' ".
					   "AND `id_utilisateur` = '".intval($client->id)."'");
				return -($qte+$qte2);
			}
	...
	{
		$res = $this->_delta_verrou($client, -$qte, $cot);
