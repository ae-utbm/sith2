...
if ( $_REQUEST["page"] == "AE" || $_REQUEST["page"] == "SG" )
{
	...
	$cts = new contents(classlink($user)." / <a href=\"moncompte.php?id_utilisateur=".$user->id."\">Compte AE</a> / Depenses ($mode) $mois / $annee");
	
	$cts->add(new sqltable(
		"listresp", 
		"Depenses", $req, "moncompte.php?id_utilisateur=".$user->id, 
		"id_facture", 
		array(
			"id_facture"=>"Facture",
			"date_facture"=>"Date",
...
elseif ( $_REQUEST["page"] == "rech" )
{
	...
	$site->start_page("services", $user->prenom . " " . $user->nom );
	$cts = new contents(classlink($user)." / <a href=\"moncompte.php?id_utilisateur=".$user->id."\">Compte AE</a> / Rechargements $mois/$annee");
...
if(!empty($report))
{
  ...
  foreach( $report as $month => $data )
  {
  	...
  	$tbl->add_row(array("$mois / $annee",
		  "<a href=\"moncompte.php?page=AE&amp;month=$month&amp;id_utilisateur=".$user->id."\">".($data["depense"]/100)."</a>",
	  	"<a href=\"moncompte.php?page=rech&amp;month=$month&amp;id_utilisateur=".$user->id."\">".($data["recharge"]/100)."</a>"),"ln$t");	
  }
...
if(!empty($report))
{
  ...
  foreach( $report as $month => $data )
  {
	  ...
	  $annee = substr($month,0,4);	
  	$tbl->add_row(array("$mois / $annee",
	  	"<a href=\"moncompte.php?page=SG&amp;month=$month&amp;id_utilisateur=".$user->id."\">".($data / 100)."</a>"),"ln$t");	
  }
