...
$comptoirs = array(0=>"-") + $site->admin_comptoirs;
...
if ( $_REQUEST["action"] == "view" )
{
  ...
  $req = new requete($site->db,
    ...
    "`cpt_mise_en_vente`.`stock_local_prod`, " .
		"(SELECT SUM(`cpt_vendu`.`quantite`) AS `ventes` FROM `cpt_vendu` WHERE `cpt_produits`.`id_produit`=`cpt_vendu`.`id_produit` GROUP BY `cpt_produits`.`id_produit`) ".
    "FROM `cpt_produits` " .
