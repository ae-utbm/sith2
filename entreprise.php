...
if (isset($_REQUEST["id_ent"]))
{
	...
	else if ( $_REQUEST["page"] == "edit" )
	{
		...
		$frm->add_text_field("ville","Ville",$entreprise->ville);
		$frm->add_text_field("cpostal","Code postal",$entreprise->cpostal);
		$frm->add_text_field("pays","Pays",$entreprise->pays);
	...
	elseif( $_REQUEST["action"] == "save" )
	{
		if ( $_REQUEST["nom"] != "" )
		{
			$entreprise->save($_REQUEST["nom"],$_REQUEST["rue"],$_REQUEST["ville"], $_REQUEST["cpostal"],
				$_REQUEST["pays"],$_REQUEST["telephone"],$_REQUEST["email"],$_REQUEST["fax"]);
		}
	}
	...
	$tbl->add_row(array("Ville",$entreprise->ville));
	$tbl->add_row(array("Code postal",$entreprise->cpostal));
	$tbl->add_row(array("Pays",$entreprise->pays));
...
if( $_REQUEST["action"] == "addent"/* && $site->user->is_in_group("gestion_ae")*/)
{
	if ( $_REQUEST["nom"] != "" )
	{
		$entreprise->add($_REQUEST["nom"],$_REQUEST["rue"],$_REQUEST["ville"], $_REQUEST["cpostal"],
			$_REQUEST["pays"],$_REQUEST["telephone"],$_REQUEST["email"],$_REQUEST["fax"]);
	}
}


$site->start_page("none","Entreprises");
...
$req = new requete($site->db,
	"SELECT `id_ent`, `nom_entreprise`, `telephone_entreprise`, `email_entreprise` " .
...
if ( $req->lines > 0 )
{
	$tbl = new sqltable(
		...
		"id_ent", 
		array("nom_entreprise"=>"Nom","telephone_entreprise"=>"Telephone","email_entreprise"=>"Adresse email"), 
...
$frm->add_text_field("ville","Ville","");
$frm->add_text_field("cpostal","Code postal","");
$frm->add_text_field("pays","Pays","France");
$frm->add_text_field("email","Adresse email","");
$frm->add_text_field("telephone","Téléphone","");
$frm->add_text_field("fax","Numéro de fax","");
$frm->add_submit("valid","Ajouter");
