...
if ( $_REQUEST["action"] == "generate" )
{
	...
	while ( $row = $req->get_row())
	{
		...
		$src = "/var/www/ae/www/ae2/images/logos/".$row['nom_unix_asso_gest'].".png";
		$logo = "/var/www/ae/www/ae2/images/logos/".$row['nom_unix_asso_gest'].".jpg";
		if ( !file_exists($logo) && file_exists($src) )
			exec(escapeshellcmd("/usr/share/php5/exec/convert $src -background white $logo"));

		if ( !file_exists($logo) )
		{
			$src = "/var/www/ae/www/ae2/images/logos/".$row['nom_unix_asso_prop'].".png";
			$logo = "/var/www/ae/www/ae2/images/logos/".$row['nom_unix_asso_prop'].".jpg";
