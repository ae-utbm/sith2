...
if ( $_REQUEST['module']=="fsearch" )
{
  ...
  echo "if ( document.getElementById('fsearchpattern').value == '".addslashes($_REQUEST["pattern"])."' ||  fsearch_display_query.length < ".strlen($_REQUEST["pattern"]).") {\n";	
	...
	$fsearch = new fsearch ( $tgtopdir, $site );
...
elseif ( $_REQUEST['module']=="explorer" )
{
  ...
  {
    $fd->_load($row);
    echo "<li><a href=\"#\" onclick=\"zd_seldir('$field','".$fd->id."','$tgtopdir'); return false;\"><img src=\"".$tgtopdir."images/icons16/folder.png\" alt=\"dossier\" /> ".htmlentities($fd->titre,ENT_COMPAT,"UTF-8")."</a><ul id=\"".$field."_".$fd->id."_cts\" style=\"display:none;\"></ul></li>";
  }
  ...
  if ( !is_null($folder->id) )
  {
    ...
    {
      $fd->_load($row);
      $img = $tgtopdir."images/icons16/".$fd->get_icon_name();
      echo "<li><a href=\"#\" onclick=\"zd_selfile('$field','".$fd->id."','$tgtopdir'); return false;\"><img src=\"$img\" alt=\"fichier\" /> ".htmlentities($fd->titre,ENT_COMPAT,"UTF-8")."</a></li>";
    }
  }
}
elseif ( $_REQUEST['module']=="complete" )
{
	...
	*/
	...
	{
		if ( $site->user->id < 1 && !count($_SESSION["Comptoirs"])) exit();
...
elseif ( $_REQUEST['module']=="usersession" )
{
	...
	if ( isset($_REQUEST["set"]) )
	{
		...
		if ( $site->user->id > 0 ) // mémorise le usersession
