...
while ($row = $events->get_row ())
{
  ...
  $cal->add_event ( "http://ae.utbm.fr". $GLOBALS["wwwtopdir"]."news.php?id_nouvelle=".$row["id_nouvelle"]."&date=".$row["date_debut_eve"], $row['titre_nvl'], $row['resume_nvl'], $st, $end, $dateonly, "http://ae.utbm.fr". $GLOBALS["wwwtopdir"]."news.php?id_nouvelle=".$row["id_nouvelle"], $row["nom_geopoint"], $row["lat_geopoint"], $row["long_geopoint"] );
}
