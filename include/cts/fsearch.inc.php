...
class fsearch extends stdcontents
{
  ...
  var $dbpg;
	...
	function fsearch ( $tgtopdir, $site )
	{
		global $topdir;
  ...
  if ( count($clefs) )
  {
    ...
    if ( $req->lines > 0 )
    {
      ...
      {
        $this->buffer .= "<li><a href=\"".$tgtopdir."pgae.php?recherche=".$pat.$clefs[$id_motclef][0]."\">".$retenu.$clefs[$id_motclef][1]."</a> (".$count." resultats)</li>";
      }
		  $this->buffer .= "</ul>";	
		}
	}
...
// Utilisateurs
if ( $site->user->id > 0 && ($site->user->utbm || $site->user->ae) )
{
	...
	$req = new requete($site->db, "SELECT COUNT(*) " .
			...
			"UNION ALL SELECT COUNT(*) " .
			"FROM `utilisateurs` " .
			"WHERE `alias_utl`!='' AND `alias_utl` REGEXP '^".$sqlpattern."' $force_sql " .
			"UNION ALL SELECT COUNT(*) " .
			"FROM `utl_etu_utbm` " .
	...
	if ( $nbutils > 0 )
	{
		...
		{
			if ( $req->lines == 1 )
				$this->redirect = $tgtopdir."user.php?id_utilisateur=".$row['id_utilisateur'];
			...
			elseif ( $row["method"] == 2 )
			{
				// En attendant de replacer le prenom avant le nom
				$nom = eregi_replace($pattern,"<b>\\0</b>",$row[0]);
			}
		...
		$this->buffer .= "</ul>";
		if ( $nbutils > 3 )
			$this->buffer .= "<p class=\"more\"><a href=\"".$tgtopdir."matmatronch/?action=simplesearch&amp;pattern=".urlencode($_REQUEST["pattern"])."\">".($nbutils-3)." autre(s) resultat(s)</a></p>";
	}
}
	...
	if ( $req->lines )
	{
		...
		while ( $row = $req->get_row() )
		{
			...
			$this->buffer .= "<li><a href=\"".$tgtopdir."asso.php?id_asso=".$row["id_asso"]."\"><img src=\"".$tgtopdir."images/icons16/asso.png\" class=\"icon\" alt=\"\" /> ".eregi_replace($pattern,"<b>\\0</b>",$row["nom_asso"])."</a></li>";	
		}
	...
	if ( $req->lines )
	{
		...
		{
			if ( $req->lines == 1 )
				$this->redirect = $tgtopdir."e-boutic/?cat=".$row['id_typeprod'];
				
			$this->buffer .= "<li><a href=\"".$tgtopdir."e-boutic/?cat=".$row['id_typeprod']."\">" .
					"<img src=\"".$tgtopdir."images/icons16/produit.png\" class=\"icon\" alt=\"\" />" .
	...
	if ( $req->lines )
	{
		...
		{
			if ( $req->lines == 1 )
				$this->redirect = $tgtopdir."news.php?id_nouvelle=".$row['id_nouvelle'];
			...
			$this->buffer .= "<li><a href=\"".$tgtopdir."news.php?id_nouvelle=".$row['id_nouvelle']."\"><img src=\"".$tgtopdir."images/icons16/nouvelle.png\" class=\"icon\" alt=\"\" /> ".eregi_replace($pattern,"<b>\\0</b>",$nom)."</a></li>";	
		}
