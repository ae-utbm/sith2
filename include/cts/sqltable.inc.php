		...
		while ( is_array($sql) ? (list($rien,$row) = each($sql)) : ($row = $sql->get_row()) )
		{
			...
			foreach ( $cols as $key => $name )
			{
			  ...
			  if ( $key == "=num" )
			  {
				...
				foreach ( $GLOBALS["entitieslinks"] as $ent )
				{
					if ( ereg("^".$ent[1]."(.*)$",$key,$reg))	
					{
						...
						if ( $id )
						{
							if ( $ent[3] )
							{
								$this->buffer .= "<a href=\"".$topdir.$ent[3]."?".$ent[0]."=$id\">";
								$this->buffer .= "<img src=\"".$topdir.$ent[2]."\" class=\"icon\" alt=\"\" />";
								$this->buffer .= "</a> ";
							}
							else
								$this->buffer .= "<img src=\"".$topdir.$ent[2]."\" class=\"icon\" alt=\"\" />";
			...
			foreach ( $actions as $key => $name )
			{
				...
				if ( file_exists( $topdir . "images/actions/" . $key.".png")   )
					$this->buffer .= "<img src=\"".$topdir . "images/actions/" . $key.".png\" alt=\"".htmlentities($name,ENT_NOQUOTES,"UTF-8")."\" title=\"".htmlentities($name,ENT_NOQUOTES,"UTF-8")."\" class=\"icon\" />";
				else
