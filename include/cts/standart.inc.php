...
class wikicontents extends contents
{
	...
	function wikicontents($title,$contents)
	{
		...
		$this->buffer = $this->wiki->transform($this->contents);
	}
...
class form extends stdcontents
{
	...
	function add_text_field ( $name, $title, $value = "", $required = false , $size = false, $fast_clean = false, $enabled = true, $text_after = null)
	{
		...
		$this->buffer .= "<div class=\"formfield\"><input type=\"text\" name=\"$name\" value=\"$value\"";
		if ( $fast_clean )
	...
	function add_user_fieldv2 ( $name, $title, $value = "", $required = false )
	{
		...
		$this->buffer .= "<div id=\"$name\" class=\"userfield\">" .
				...
				"</div>" .
				"<div id=\"".$name."_static\" class=\"staticbox\">" .
				"(personne)" .
				"</div>" .
				"<div class=\"button\">" .
	...
	function add_date_field ( $name, $title, $value = -1, $required = false , $tipp = false, $enabled = true)
	{
		...
		if ( $tipp )
			$this->_add_tipp( $tipp );
		$this->buffer .= "<a href=\"javascript:openCalendar('".$topdir."','".$this->name."','$name','date')\">";
		$this->buffer .= "<img src=\"".$topdir."images/calendar.png\">";
		$this->buffer .= "</a>";
		$this->buffer .= "</div>\n";
		$this->buffer .= "</div>\n";
	}
	...
	{	
		...
		$this->buffer .= "\" />";
		$this->buffer .= "<a href=\"javascript:openCalendar('".$topdir."','".$this->name."','$name','datetime')\">";
		$this->buffer .= "<img src=\"".$topdir."images/calendar.png\">";
		$this->buffer .= "</a>";		
		$this->buffer .= "</div>\n";
		$this->buffer .= "</div>\n";
	 ...
	 */		
	function add_radiobox_field ( $name, $title=false, $values, $value=false , $disabled=false, $required = false, $imgs=array()  )
	{
		global $topdir;
	...
	/** Selecteur d'une entité definie dans entitieslinks
	 * @param $name		Nom du champ
	 * @param $title	Libéllé du champ
	...
	function add_entity_select ( $name, $title, $db, $entityclass, $value=false, $none=false, $conds=array())
	{
		...
		if ( $entityclass == "group") // cas ultra particulier
		{
		...
		else
		{
			...
			else if ( $GLOBALS["entitieslinks"][$entityclass][4] )
				$sql = "SELECT `".$GLOBALS["entitieslinks"][$entityclass][0]."`,`".$GLOBALS["entitieslinks"][$entityclass][1]."` FROM `".$GLOBALS["entitieslinks"][$entityclass][4]."`";	
			else
				return; // non supporté
		...
		$prefix="";
		if ( $GLOBALS["entitieslinks"][$entityclass][2] )
			$prefix = "<img src=\"".$topdir.$GLOBALS["entitieslinks"][$entityclass][2]."\" class=\"icon\" alt=\"\" /> ";
...
class tabshead
{
	...
	function html_render()
	{
		...
		foreach ($this->entries as $entry)
		{
			...
			$this->buffer .= "><a href=\"" . htmlentities($topdir . $entry[1],ENT_NOQUOTES,"UTF-8") . "\"";
			if ($this->sel == $entry[0])
				$this->buffer .= " class=\"selected\"";
...
if ( isset($_REQUEST["magicform"]) )
{
	...
	if ( $_REQUEST["magicform"]["date"] )
	{
		foreach ( $_REQUEST["magicform"]["date"] as $name => $value )
			$_REQUEST[$name] = datetime_to_timestamp($value);
	}
	...
	if ( $_REQUEST["magicform"]["boolean"] )
	{
		...
		{		
			$_REQUEST[$name] = ($value == "true");
			if ( ereg("(.*)\|(.*)",$name,$regs) )	
				$_REQUEST[$regs[1]][$regs[2]] = ($value == "true");
		}
	}
