...
class userinfov2 extends stdcontents
{
  ...
  $this->buffer .= "<h2 class=\"nom\">".htmlentities($user->prenom." ".$user->nom,ENT_COMPAT,"UTF-8")."</h2>\n";
    ...
    if ( $ville->is_valid() || $user->addresse )
    {
      ...
      if ( $ville->is_valid() )
			  $this->buffer .= "<br/>".$ville->get_html_link()." (".sprintf("%05d", number_format($ville->cpostal, 0, ',', ' ')).")";
    ...
    if ( $display == "full" )
    {
      ...
      if ( $ville_parents->is_valid() || $user->adresse_parents )
      {
        ...
        if ( $ville_parents->is_valid() )
          $this->buffer .= "<br/>".$ville_parents->get_html_link()." (".sprintf("%05d", number_format($ville_parents->cpostal, 0, ',', ' ')).")";
