...
class galaxy
{
  ...
  function cycle ( $detectcollision=false )
  {
    ...
    new requete($this->dbrw,"UPDATE galaxy_star SET
				dx_star = IFNULL(dx_star + LEAST(100,max_tense_star)*(x_star - '$center_x')/( (POW(x_star - '$center_x' , 2) + POW(y_star - '$center_y' , 2))*sum_tense_star),0), 
				dy_star = IFNULL(dy_star + LEAST(100,max_tense_star)*(y_star - '$center_y')/( (POW(x_star - '$center_x' , 2) + POW(y_star - '$center_y' , 2))*sum_tense_star),0)");
