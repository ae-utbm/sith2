...
function enumerates_groups($db)
{
	if ( isset($GLOBALS["groupscache"]) )
				$values = $GLOBALS["groupscache"];

	else
	{
		$req = new requete($db, "SELECT `id_groupe`,`nom_groupe` FROM `groupe` ORDER BY `nom_groupe`");		
		...
		$values[10001] = "utbm";
		$values[10002] = "ancien_etudiant";
		$values[10003] = "etudiant";
