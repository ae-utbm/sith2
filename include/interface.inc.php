...
require_once($topdir . "include/utilisateur.inc.php");
...
class interfaceweb
{
	...
	function get_textbox ($nom_boite,$form = 1)
	{
		...
		if ( $form == 0 )
		{
			$structuredText = new wiki2xhtml();
			$text = $structuredText->transform($ret[0]);
			return $text;
		}
		return $ret[0];
	}
	...
	function end_page () // <=> html_render
	{
		...
		echo "<title>Association des Etudiants de l'UTBM | ".$this->title."</title>\n";
		echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"" . $topdir . "themes/default/css/site.css\" title=\"AE2-NEW Base\" />\n";
		foreach ( $this->extracss as $url ) 
			echo "<link rel=\"stylesheet\" type=\"text/css\" href=\"" . htmlentities($topdir . $url,ENT_NOQUOTES,"UTF-8"). "\" />\n";
		...
		echo "<link rel=\"SHORTCUT ICON\" href=\"" . $topdir . "favicon.ico\" />\n";
		echo "<script type=\"text/javascript\" src=\"" . $topdir . "js/site.js\">var site_topdir='$topdir';</script>\n";
		echo "<script type=\"text/javascript\" src=\"" . $topdir . "js/ajax.js\"></script>\n";
		echo "<script type=\"text/javascript\" src=\"" . $topdir . "js/dnds.js\"></script>\n";
		echo "</head>\n";
		...
		if (!$this->compact )
		{
			...
			echo "<form action=\"".$topdir."fsearch.php\" method=\"POST\">";
			echo "<input type=\"text\" id=\"fsearchpattern\" name=\"pattern\" onblur=\"fsearch_stop_delayed();\" onkeyup=\"fsearch_keyup(event,'$topdir');\" value=\"\" />\n";
			echo "</form>";
			echo "<div class=\"fend\"></div></div>\n";
			
			echo "<div id=\"fsearchres\"></div>\n";

			// Cas trés exceptionnel du congrés, il est **trés** préférable de faire toute la thématisation en css
			// => même ce cas particulier aurais pu être traité uniquement en css
			if ( in_array ("css/congres.css",$this->extracss) )
			{
			  echo "<div id=\"logo\">";
			  echo "<img src=\"". $topdir ."images/themes/congres/bulles.png\" border=\"0\" style=\"float: right; position: relative; top: 0px; padding: 0px; margin: none;\" height=\"70\">";
			  echo "<a href=\"http://ae.utbm.fr\"><img src=\"" . $topdir ."images/themes/congres/AE.png\" alt=\"Logo AE\"/></a>";
			  echo "<img src=\"". $topdir ."images/themes/congres/titre.png\" border=\"0\" style=\"position: absolute; float: left; top: 0px; left: 200px; margin: none; padding: none;\">";
			  echo "</div>";			
			}
			else	// *** truc standart *** 			
			  echo "<div id=\"logo\"><a href=\"http://ae.utbm.fr\"><img src=\"" . $topdir ."images/Ae.jpg\" height=\"60\" width=\"218\" alt=\"Logo AE\"/></a></div>";					
		...
		foreach ($this->tab_array as $entry)
		{
			...
			echo "><a id=\"tab_".$entry[0]."\" href=\"" . $topdir . $entry[1] . "\"";
			/*if ($this->section == $entry[0])
				echo " class=\"selected\"";*/
			echo " title=\"" . $entry[2] . "\">".
			  $entry[2] . "</a></span>\n";
		...
		if ( $links )
		{
			...
			foreach ( $links as $entry )
			{
				if ( ereg("http://(.*)",$entry[0]) )
					...
					echo "<a href=\"".$topdir.$entry[0]."\">".$entry[1]."</a>\n";
			}
		...
		{
			$i++;
			echo "<div class=\"article\" id=\"cts$i\">\n";
		...
		echo "<a href=\"". $topdir ."article.php?name=legals\">AE UTBM</a> - <a href=\"/equipeinfo\">Equipe info</a>";
		echo "<br/>\n";     
