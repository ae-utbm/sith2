...
require_once($topdir . "include/sondage.inc.php");
...
class site extends interfaceweb
{
  ...
  function db_error ()
  {
    ...
    echo " <head>\n";
    echo "  <title>Association des &Eacute;tudiants de l'UTBM - Erreur</title>\n";
    echo "  <link rel=\"stylesheet\" href=\"css/dberror.css\" title=\"AE\" />\n";
    echo " </head>\n\n";

    echo " <body id=\"dberror\">\n";
  ...
  function get_wc ()
  {
    ...
    $last_m = @filemtime("/var/www/ae/www/var/img/webcam.mini.png");

    if (file_exists("/var/www/ae/www/var/img/webcam.mini.png"))
      $imgsrc = $topdir . "var/img/webcam.mini.png?".$last_m;
    else
	...
	function get_meteo ()
	{
		...
		$cts = new contents("La meteo de Belfort");
		...
		for($i=0;$i<2;$i++)
		{
			...
			if ( strpos($day["weather"],"Sunny") !== false  )
				$icon="soleil.png";
			elseif ( strpos($day["weather"],"Showers")!== false || strpos($day["weather"],"Rain") !== false || strpos($day["weather"],"Stroms") !== false )
				$icon="pluvieux.png";
			else
				$icon="nuageux.png";
			...
			switch($day["day"])
			{
				case "Today"	: $day["day"] = "Aujourd'hui"; break;
				case "Mon"	: $day["day"] = "Lundi"; break;
				case "Tue"	: $day["day"] = "Mardi"; break;
				case "Wed"	: $day["day"] = "Mercredi"; break;
				case "Thu"	: $day["day"] = "Jeudi"; break;
				case "Fri"	: $day["day"] = "Vendredi"; break;
				case "Sat"	: $day["day"] = "Samedi"; break;
				case "Sun"	: $day["day"] = "Dimanche"; break;
			}
			
			$cts->puts("<div class=\"meteoday\">");
			$cts->puts("<div class=\"weather\"><img src=\"".$topdir."images/meteo/".$icon."\" alt=\"".$day["weather"]."\" class=\"meteoimg\" /></div>");
			$cts->puts("<div class=\"day\">".$day["day"]."</div>");
			$cts->puts("<div class=\"tempmax\">Max: ".$day["max"]."&deg;C</div>");
			$cts->puts("<div class=\"tempmin\">Min: ".$day["min"]."&deg;C</div>");
    ...
    if (  is_null($this->user->date_maj) )
        ...
        */
		...
		if ( ($cotiz->id > 0) && ($cotiz->date_fin_cotis < time()) && (time()-$cotiz->date_fin_cotis < (30*24*60*60)) )
		{
	...
	function get_sondage()
	{
		...
		$frm->add_radiobox_field ( "numrep", "", $resp );
	...
	function get_connection_contents ()
	{
      ...
      return $frm;
		...
		$cts->add_paragraph("<br><a href=\"".$topdir."comptoir/moncompte.php\">Compte AE : ".($this->user->montant_compte/100)." Euros</a>");
		...
		if ( $nb )
			$sublist->add("<a href=\"".$topdir."reservation.php\"><b>Mes reservations de salles : $nb en attente</b></a>");
	...
	function get_anniv_contents ()
	{
		...
		if ($req->lines > 0)
		{
            ...
            $cts->puts("</ul>\n");
				...
				$count++;
				$cts->puts ("<li><a href=\"". $topdir ."user.php?id_utilisateur=". $res['id_utilisateur'] .
				 "\" title=\"". $res['prenom_utl'] . " " . $res['nom_utl'] . "\">" . $nom . "</a></li>\n");
			}
			$cts->puts("</ul>\n");
			//$cts->puts("</ul>\n", -1);
		}
