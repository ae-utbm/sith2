...
elseif ( $_REQUEST["action"] == "search" )
{
    ...
    if ( !$is_admin )
    {
      ...
      $sqlnb .= "AND `publique_utl`='1'";		
		...
		if ( $_REQUEST["display_by"] == 2 ) // par PAGE
		{
			$npp=18;
		...
		elseif ( $_REQUEST["display_by"] == 2 )
		{
			...
			while ( $row = $req->get_row() )
			{
				...
				//$tbl->add(new userinfo($user,true,false,true));
				//$gal->add_item(new userinfo($user,true,false,true));
				$gal->add_item(new userinfov2($user));
			}
			//$tbl->puts("<div class=\"clearboth\"></div>\n");
