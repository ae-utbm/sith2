...
/*elseif ( isset($_REQUEST["userpattern"] ) )
{
  $user = new utilisateur($site->db);
  $req = $user->load_by_pattern($_REQUEST["userpattern"]);
  if ( $user->id < 0 )
  {
    ...
    if ($req->lines==0)
      $tbl = new contents("Non trouvé...");
    else
      $tbl = new sqltable(
        "listresult",
        "Plusieurs personnes correspondents", $req, "user.php",
        "id_utilisateur",
        array("nom_utl"=>"Nom","prenom_utl"=>"Prenom","surnom_utbm"=>"Surnom",
        "date_naissance_utl"=>"Date de naissance","promo_utbm"=>"Promo"),
        array("view"=>"Voir les photos"), array(), array( )
        );
...
$tabs = array(array(false,"user.php?id_utilisateur=".$user->id, "Informations"),
      ...
      array(true,"sas2/user.php?id_utilisateur=".$user->id, "Photos"));

if (  $user->id==$site->user->id || $site->user->is_in_group("gestion_ae") )
{
  $tabs[]=array(false,"reservation.php?id_utilisateur=".$user->id, "Reservations");
...
$cts->add(new tabshead(array(
  array("","sas2/user.php?id_utilisateur=".$user->id,"Photos"),
  array("stats","sas2/user.php?see=stats&id_utilisateur=".$user->id,"Statistiques")),
...
else
{
  ...
  while ( $row = $req->get_row())
  {
    ...
    $gal->add_item("<a href=\"./?id_photo=".$row['id_photo']."\"><img src=\"$img\" alt=\"Photo\"></a>");
  }
  if ( $gal )
    $cts->add($gal,true);
