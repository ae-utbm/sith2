...
$site->set_admin_mode();
	...
	$factured_infos = array ('name' => "AE - UTBM",
			 'addr' => array("6 Boulevard Anatole France",
					 "90000 BELFORT"),
			 'logo' => "/var/www/ae/www/ae2/images/Ae-blanc.jpg");
	...
	$titre = utf8_decode("Facture Administration");
	
/*$sql = new mysql('ae_read_only', 'ljjTutG', 'localhost', 'ae');

	$query = new requete ($sql, "SELECT EXTRACT(YEAR_MONTH FROM `date`) AS `month`,`nom`,`prix`,SUM(`quantite`) AS `qte`,SUM(`prix`*`quantite`) AS `total` FROM `sa_success` WHERE `id_produit` IN ('54','55') GROUP BY `nom`");*/



	/*while ($line = $query->get_row ())
	{*/
	  $lines[] = array('nom' => "Mat'Matronch 2006-2007",
			   'quantite' => 60,
			   'prix' => 1000,
			   'sous_total' => 60000);
	  	$ref = "200610";
		$date_facturation = date("d/m/Y", strtotime("now"));
	//}
