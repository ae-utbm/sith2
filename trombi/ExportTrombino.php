...
if ($site->user->is_in_group("root")) {
    ...
    if ($req->lines > 0) {
        while ($row = $req->get_row()) {
            ...
            try {
                $req_comment = new requete($site->db, "select commentaire, utl_etu_utbm.surnom_utbm from trombi_commentaire join utl_etu_utbm on trombino_commentaires.id_commentateur=utl_etu_utbm.id_utilisateur where id_commente = " . $id_user);
