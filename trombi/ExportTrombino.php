...
if ( $site->user->is_in_group("root") )
{
    ...
    if ($req->lines >0){
        while($row = $req->get_row()){
            ...
            $req_fillots= new requete($site->db,"SELECT utl_etu_utbm.surnom_utbm from parrains where id_utilisateur = ".$row["id_utilisateur"]." join utl_etu_utbm on parrains.id_utilisateur_fillot=utl_etu_utbm.id_utilisateur");
            while($row_fillots = $req_fillots->get_row()){
                 $result.="<fillot>".$row_fillots["surnom_utbm"]."</fillot>";
            }
            $req_parrains= new requete($site->db,"SELECT utl_etu_utbm.surnom_utbm from parrains where id_utilisateur_fillot = ".$row["id_utilisateur"]." join utl_etu_utbm on parrains.id_utilisateur=utl_etu_utbm.id_utilisateur");
