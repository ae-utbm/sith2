...
if ( ($_REQUEST["action"] == "delete") && $site->user->is_in_group("gestion_ae") && ( $site->user->is_in_group("root") || $site->user->id != $user->id ) )
{
		...
		if ( $fact->id > 0 )
    {
      ...
      $site->log("Annulation d'une facture","Annulation de la facture N° ".$fact->id.", d'un montant de ".$fact->montant/100."€ du ".date("Y-m-d H:i",$fact->date)." débitée à ".$user_client->nom." ".$user_client->prenom." (id : ".$user_client->id.") par ".$user_vendeur->nom." ".$user_vendeur->prenom." (id : ".$user_vendeur->id.")","Comptes AE",$site->user->id);
			$fact->annule_facture();
    }
	}
